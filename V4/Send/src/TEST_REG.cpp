...
static const char sccsid[]      = "@(#) $Workfile: TEST_REG.CPP $$ $Revision: 2 $$";
	...
	const char			*fmsg )				// Fail message (optional).
{
	...
	if ( ~im.pb_mask & ( iv.pb ^ ireg.get_pb() ) )
	{
		...
		//k fputs( "      ERROR PB    - Hardware ", stdout );
		printf( "      ERROR PB    - Hardware " );
//k 		print_byte( ireg.get_pb(), im.pb_mask );
		printf( " (0x%02x) != Vector ", ireg.get_pb() );
//k 		print_byte( iv.pb, im.pb_mask );
		printf( " (0x%02x)\n", iv.pb );
        retval	=	FAIL;
	}
	...
	if ( ~im.pc_mask & ( iv.pc ^ ireg.get_pc() ) )
	{
		...
		//kfputs( "      ERROR PC    - Hardware ", stdout );
		printf( "      ERROR PC    - Hardware " );
		//kprint_byte( ireg.get_pc(), im.pc_mask );
		printf( " (0x%02x) != Vector ", ireg.get_pc() );
		//kprint_byte( iv.pc, im.pc_mask );
		printf( " (0x%02x)\n", iv.pc );
        retval	=	FAIL;
	}
	...
	if ( ~im.s0t_mask & ( iv.s0t ^ ireg.get_s0t() ) )
	{
		...
		//kfputs( "      ERROR S0T   - Hardware ", stdout );
		printf( "      ERROR S0T    - Hardware " );
		//kprint_byte( ireg.get_s0t(), im.s0t_mask );
		printf( " (0x%02x) != Vector ", ireg.get_s0t() );
		//kprint_byte( iv.s0t, im.s0t_mask );
		printf( " (0x%02x)\n", iv.s0t );
		retval	=	FAIL;
	}
	...
	if ( ~im.s0h_mask & ( iv.s0h ^ ireg.get_s0h() ) )
	{
		...
		//kfputs( "      ERROR S0H   - Hardware ", stdout );
		printf( "      ERROR S0H    - Hardware " );
		//kprint_byte( ireg.get_s0h(), im.s0h_mask );
		printf( " (0x%02x) != Vector ", ireg.get_s0h() );
		//kprint_byte( iv.s0h, im.s0h_mask );
		printf( " (0x%02x)\n", iv.s0h );
		retval	=	FAIL;
	}
	...
	if ( ~im.s1t_mask & ( iv.s1t ^ ireg.get_s1t() ) )
	{
		...
		//kfputs( "      ERROR S1T   - Hardware ", stdout );
		printf( "      ERROR S1T    - Hardware " );
		//kprint_byte( ireg.get_s1t(), im.s1t_mask );
		printf( " (0x%02x) != Vector ", ireg.get_s1t() );
		//kprint_byte( iv.s1t, im.s1t_mask );
		printf( " (0x%02x)\n", iv.s1t );
		retval	=	FAIL;
	}
	...
	if ( ~im.gen_mask & ( iv.gen ^ ireg.get_gen() ) )
	{
		...
		//kfputs( "      ERROR GEN   - Hardware     ", stdout );
		printf( "      ERROR GEN    - Hardware " );
		//kprint_nibble( ireg.get_gen(), im.gen_mask );
		printf( " (0x%02x) != Vector     ", ireg.get_gen() & 0x0f );
		//kprint_nibble( iv.gen, im.gen_mask );
		printf( " (0x%02x)\n", iv.gen & 0x0f );
        retval	=	FAIL;
	}
