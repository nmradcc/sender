 ...
 * Coded by:             K. West
 *
 * Current version:      $Revision: 48 $
 * Last Modified on:     $Modtime: 2/12/19 4:46p $
 *               by:     $Author: Kenw $
 * Last checkin on:      $Date: 2/12/19 9:23p $
 *
 *****************************************************************************
 *
 * DESCRIPTION:
...
extern Send_reg 			Dcc_reg;
#endif


static const char sccsid[]      = "@(#) $Workfile: DEC_TST.CPP $$ $Revision: 48 $$";
...
const int		SP_TEST_MIN		= 14;				// Min. test speed.
                                                    ...
                                                    //  mode)
const int		PKT_REP_MIN		= 10;				// Min. pkt repeat count.
...
const Dec_clk	Dec_tst::dclk_tbl[] = {
	...
	{ DECODER_0T_MAX,                100,   116,  	"Max Decoder Neg 0"	   },
	{ DECODER_0T_MAX, DECODER_0T_MAX-100,   116,  	"Max Decoder Pos 0"    }
};
	...
	FILE		*ofp )						// Output file stream.
{
	...
	m_tst_name[0]	=	'\0';
	fprintf( ofp,
			"0x%08lx  %-28s 1T margin test.\n", bit_msk, m_tst_name );
	bit_msk <<= 1;
	fprintf( ofp,
			"0x%08lx  %-28s 1H duty cycle test.\n", bit_msk, m_tst_name );
	bit_msk <<= 1;
	fprintf( ofp,
			"0x%08lx  %-28s Ramp test.\n", bit_msk, m_tst_name );
	bit_msk <<= 1;
	...
	for ( i = 0; i < ames_tbl_size; i++, bit_msk <<= 1 )
	{
		...
		fprintf( ofp,
			"0x%08lx  %-28s Packet acceptance test.\n",
			bit_msk, m_tst_name );
	}

	m_tst_name[0]	=	'\0';
	fprintf( ofp,
			"0x%08lx  %-28s Bad address test.\n", bit_msk, m_tst_name );
	bit_msk <<= 1;
	fprintf( ofp,
			"0x%08lx  %-28s Bad bit test.\n", bit_msk, m_tst_name );
	bit_msk <<= 1;
	...
	{
		fprintf( ofp,
			"0x%08lx  %-28s Single stretched 0 test.\n",
			bit_msk, str0_tbl[i].msg );
	}

	m_tst_name[0]	=	'\0';
	fprintf( ofp,
			"0x%08lx  %-28s Truncated packet test.\n", bit_msk, m_tst_name );
	bit_msk <<= 1;

	m_tst_name[0]	=	'\0';
	fprintf( ofp,
			"0x%08lx  %-28s Prior packet test.\n", bit_msk, m_tst_name );
	bit_msk <<= 1;

	m_tst_name[0]	=	'\0';
	fprintf( ofp,
			"0x%08lx  %-28s 6 prior byte test.\n", bit_msk, m_tst_name );

	bit_msk <<= 1;

	m_tst_name[0]	=	'\0';
	fprintf( ofp,
			"0x%08lx  %-28s 1 ambiguous bit test.\n", bit_msk, m_tst_name );

	bit_msk <<= 1;

	m_tst_name[0]	=	'\0';
	fprintf( ofp,
			"0x%08lx  %-28s 2 ambiguous bits test.\n", bit_msk, m_tst_name );
	...
	for ( i = 0, bit_msk =  1UL; i < dclk_size; i++, bit_msk <<= 1 )
	{
		fprintf( ofp, "0x%08lx  %5hu  %5hu  %5hu    %s.\n",
	...
	Rslt_t			&tst_rslt )				// Decoder test result.
{
	...
	printf(		"Completed booster warm up\n" );
		...
		for ( i = 0, active = false; i < 2; i++, active =  true )
		{
			...
			if ( out_id == 1 && active == true )
			{
				...
				if ( (tgen & 0x01) != test_expect )
				{
					...
					tst_rslt	=	FAIL;
					ERRPRINT( my_name, LOG_WARNING,
						"Test FAILED:  Out_id %1d, Output %3s",
						out_id,
						active ? "ON" : "OFF" );
				}
			}
	...
	u_int			idle_cnt )				// Count of idles to use.
{
	...
	{
		dcc_bits2.put_func_grp_pkt(	Args.get_decoder_address(),
									GRP_1, 0, pre_cnt );
	}
    else if ( Args.get_decoder_type() == DEC_SIG )
    {
	...
	Rslt_t			&tst_rslt )					// Decoder test result.
{
	...
	/*
	 *	For accessory decoders, set up a special initialization command
	 *	rather than using a fail safe reset.  This is done since accessory
	 *	decoders generally leave there output alone on reset.
	 */
	...
	for ( addr = addr_min; addr <= addr_max + 1; addr++ )
	{
		...
		{
			dcc_bits2.put_func_grp_pkt(	send_addr,
									GRP_1, 0, pre_cnt );
		}
		else // Accessory decoder.
		{
			dcc_bits2.put_acc_pkt(		send_addr, true, 0, pre_cnt );
		}
		dcc_bits2.put_idle_pkt( BEST_IDLE );
		dcc_bits2.put_1s(1).done();

		OUT_PC( PC_POS_UNDERCLRL, 0 );
		if ( Args.get_decoder_type() != DEC_ACC )	// Send Fail Safe Rst cmd.
		{
			Dcc_reg.send_pkt( fsoc, "Send fail safe start up sequence." );
			send_filler();
		}
		else										// Send Accessory init cmd.
		{
			for ( j = 0; j < pkt_rep_cnt;  j++ )
			{
				Dcc_reg.send_pkt( dcc_bits3,
					"Send bad address accessory init packet." );
			}
		}
		...
		{
        	Dcc_reg.set_scope( true );
			Dcc_reg.send_pkt( dcc_bits2, "Send bad address trigger command." );
		}
	...
	Rslt_t			&tst_rslt )				// Decoder test result.
{
	...
	/*
	 *	Preset decoder to reverse with lamp off or
	 *  preset accessory decoder to the reverse position.
	 */
	dcc_bits.clr_in();
	dcc_bits.put_0s(1);	// Make sure a 0 is ahead of the preamble.
	...
	for ( i = first_bit; i <= last_bit; i++ )
	{
		...
		else										// Send Accessory init cmd.
		{
			...
			{
				Dcc_reg.send_pkt( dcc_bits3,
					"Send bad bit accessory init packet." );
			}
		}
		...
		{
            Dcc_reg.set_scope( true );
			Dcc_reg.send_pkt( dcc_bits2, "Send bad bit trigger command." );
		}
	...
	Rslt_t			&tst_rslt )				// Decoder test result.
{
	...
	{
		dcc_bits.put_acc_pkt(Args.get_decoder_address(), true, 1, BEST_PRE );
	}
	dcc_bits.put_idle_pkt( BEST_IDLE );
	dcc_bits.put_1s(1).done();
		  ...
		  for ( pre_bits = PRE_BITS; pre_bits <= PRE_BITS + 10; pre_bits++ )
		  {
			 ...
			 */
			 ...
			 for ( i = 0; i < test_repeats; i++ )
			 {
				...
				{
					Dcc_reg.send_pkt( dcc_bits, "Send prior packet preset." );
				}
				...
				// Trigger.
				Dcc_reg.send_pkt( dcc_bits2, "Send prior packet trigger." );
	...
	Rslt_t			&tst_rslt )				// Decoder test result.
{
    ...
    {
    	dcc_bits3.put_func_grp_pkt(	Args.get_decoder_address(),
        							GRP_1, 0, pre_cnt );
    }
	else // Accessory decoder.
	{
	...
	u_short			clk1t )					// Clock 1T value.
{
	 ...
	 */
	filler_idles	=	(int)( fill_usec /
						(	(u_long)clk0t * (u_long)(		IDLE_0S
														+	FILLER_0S )
						+ 	(u_long)clk1t * (u_long)(		IDLE_1S
														+	FILLER_1S ) ) );
	if ( filler_idles < PKT_REP_MIN )
	{
		filler_idles	=	PKT_REP_MIN;
	}
	...
	pkt_rep_cnt		=	(int)( fill_usec /
						...
						+ 	(u_long)clk1t * (u_long)(		IDLE_1S
														+	CMD_1S
														+	1 ) ) );
	if ( pkt_rep_cnt < PKT_REP_MIN )
	{
		pkt_rep_cnt		=	PKT_REP_MIN;
	}

	if ( Dcc_reg.get_log_pkts() )
	{
		TO_PKT_LOG( "!Dec_tst::calc_filler() Idle cnt %d, Preset cnt %d.\n",
...
Dec_tst::send_filler( void )
{
	...
	if ( Dcc_reg.get_log_pkts() )
	{
		TO_PKT_LOG( "!Dec_tst::send_filler() Sending %d idles.\n", filler_idles );
	}
