 ...
 * Last Modified on:     $Modtime: 4/28/17 8:46p $
 *               by:     $Author: Kenw $
 * Last checkin on:      $Date: 4/28/17 8:49p $
 *
 *****************************************************************************
 *
 * DESCRIPTION:
...
static const char sccsid[]      = "@(#) $Workfile: SELF_TST.CPP $$ $Revision: 7 $$";
...
Self_tst::print_stats( void ) const
{
	...
	if ( get_tst_cnt() > 0L )
	{
//k		ERRPRINT( my_name, LOG_INFO,
//k			"Passed %lu test cycles, Failed %lu test cycles, test count %lu",
//k			get_tst_pass(), get_tst_fail(), get_tst_cnt() );
//k		ERRPRINT( my_name, LOG_INFO,
//k			"Failed %lu vector, %lu dynamic, %lu both, last vector %d",
//k			get_v_tst_fail(),get_d_tst_fail(),get_vd_tst_fail(),p_vect-1 );
//k		ERRPRINT( my_name, LOG_INFO,
//k			"Dynamic 0 - min %lu, avg %lu, max %lu",
//k			get_min_0(), get_avg_0(), get_max_0() );
//k		ERRPRINT( my_name, LOG_INFO,
//k			"Dynamic 1 - min %lu, avg %lu, max %lu",
//k			get_min_1(), get_avg_1(), get_max_1() );
		printf(
...
Self_tst::test_send( void )
{
	...
	if ( vector_test() !=  OK )
	{
//k		ERRPRINT( my_name, LOG_ERR, "Vector self test FAILED" );
		retval	=	FAIL;
	}
	else
	{
	...
	if ( dynamic_test() != OK )
	{
//k		ERRPRINT( my_name, LOG_ERR, "Dynamic self test FAILED" );
		if ( retval == FAIL )
		{
			++vd_tst_fail;
		}
...
Self_tst::vector_test( void )
{
	...
	// (0) First reset test.
//k	dcc_reg.rst_8255();						// Reset board.
//k	if ( do_test() != OK )
//k	{
//k		return ( FAIL );
//k	}
...
//k	outportb( PA, 0x55 );
	 ...
	 */
//k	outportb( PA, 0xff );	// Set up all 1s Byte for next cycle.
	t_vect	=	p_vect;
	for ( i = 2; i <= 5; i++ )
	{
		p_vect	= t_vect;
...
{
//k	struct time	sw_time;					// Test start time.
	u_long		d_time = 0;						// Time the test ran.

	dcc_reg.init_send();					// Initialize the hardware.
	dcc_reg.start_clk();					// Start oscillator clock.
	OUT_PC( PC_POS_UNDERCLRL, 0 );			// Clear underflow.

//k	start_watch( sw_time );
...
{
//k	struct time	p_time;						// Present time.
...
//k	gettime ( &p_time );
	 ...
	 */
//k	start_time	=		((u_long)sw_time.ti_hund)
//k					+	((u_long)sw_time.ti_sec   	* 100L)
//k					+	((u_long)sw_time.ti_min		* 100L * 60L)
//k					+	((u_long)sw_time.ti_hour	* 100L * 60L * 60L );

	/*
	 *	Find the end time in 100ths of a second.
	 */
//k	end_time	=		((u_long)p_time.ti_hund)
//k					+	((u_long)p_time.ti_sec		* 100L)
//k					+	((u_long)p_time.ti_min		* 100L * 60L)
//k					+	((u_long)p_time.ti_hour		* 100L * 60L * 60L );

	/*
	 *	Handle the day rollover case.
	 */
//k	if ( ( delta_time = end_time - start_time ) < 0 )
//k	{
//k		delta_time += 100L * 60L * 60L * 24L;
//k	}
