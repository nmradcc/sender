...
///#pragma diag_suppress=Pe177     /* Avoid warning about variable declared but never referenced. */
...
//0};
#pragma diag_default=Pe177  	/* restore warning */
...
#include <ctype.h>
//tp #include <sys/types.h>
#include <signal.h>
//tp #include <sys/stat.h>
#include <errno.h>
...
#include "gzip.h"
	...
	#include "lzw.h"
#endif
//#pragma diag_suppress=Pa050     /* Avoid warning */
#include "revision.h"
#include "getopt.h"
//#pragma diag_default=Pa050  	/* restore warning */
...
#ifdef NO_OFF_T
  typedef long off_t;
  off_t seek OF((FILEHANDLE fd, off_t offset, int whence));
#endif
...
int ztest = 0;         /* test .gz file integrity */
...
long ztime_stamp;      /* original time stamp (modification time) */
//k DATETIME smxtime_stamp;      /* structure of date-time from the smx file system */
...
FILEINFO istat;         /* status for input file */
FILEHANDLE  ifd;           /* input file descriptor */
FILEHANDLE  ofd;           /* output file descriptor */
...
local int  do_stat      OF((char *name, FILEINFO *sbuf));
local char *get_suffix  OF((char *name));
local int  get_istat    OF((char *iname, FILEINFO *sbuf));
local int  make_ofname  OF((void));
local int  same_file    OF((FILEINFO *stat1, FILEINFO *stat2));
local int name_too_long OF((char *name, FILEINFO *statb));
local void shorten_name  OF((char *name));
local int  get_method   OF((FILEHANDLE in));
local void do_list      OF((FILEHANDLE ifd, int method));
local int check_ofname  OF((void));
local void copy_stat    OF((FILEINFO *ifstat));
local void do_exit      OF((int exitcode));
      int gzmain        OF((int argc, char **argv));
//int (*work) OF((FILEHANDLE infile, FILEHANDLE outfile)) = zip; /* function to call */
int (*work) OF((FILEHANDLE infile, FILEHANDLE outfile)) = NULL; /* function to call */

#ifndef NO_DIR
local void treat_dir    OF((char *dir));
#endif
#ifndef NO_UTIME
local void reset_times  OF((char *name, FILEINFO *statb));
#endif
...
//    char **argv;
{
    ...
    /* And get to work */
    if (file_count != 0) {
	if (to_stdout && !ztest && !list && (!decompress || !ascii)) {
...
//    int optc;           /* current option */
	...
	// things put in by tmp for the smx environment...
	progname = cGzipConst;
	optind = 0;
	work = zip;
	// ...end of things
	...
	file_count = 1;
	ascii = 1;
	maxbits = 8;	// don't know a good number for this
	decompress = 0;
//    list = decompress = to_stdout = 1;
	...
	quiet = 0;
	verbose = 1;
	// for test 	    ztest = decompress = to_stdout = 1;
	do_lzw = 0;
/*	
    while ((optc = getopt_long (argc, argv, "dqmv",
				longopts, (int *)0)) != EOF)
	{
		switch (optc)
		{
...
#ifdef LZW
    if (do_lzw && !decompress) work = lzw;		//tp we do not support lzw
#endif
...
int gzsingle (char* filename, int decomp)	// (int argc, char **argv)
{

	// things put in by tmp for the smx environment...
	while (work) {count (10, ticks, INF);};	// task-safety for all the globals
	progname = cGzipConst;
	optind = 0;
	work = zip;
	// ...end of things
...
local void treat_stdin()
{
    ...
    if (!ztest && !list && (!decompress || !ascii)) {
		SET_BINARY_MODE(fileno(stdout));
    }
    strcpy(ifname, "stdin");
 ...
 */
extern unsigned long DateTimeToSeconds(struct tm* pDateTime);
unsigned long ConvertFileInfoTimeToLong(DATETIME* pstat)
{
	struct tm file_mtime;
	file_mtime.tm_year = pstat->wYear + 1980;
	file_mtime.tm_mon = pstat->wMonth;
	file_mtime.tm_mday = pstat->wDay;
	file_mtime.tm_hour = pstat->wHour;
	file_mtime.tm_min = pstat->wMinute;
	file_mtime.tm_sec = pstat->wSecond;
	return DateTimeToSeconds(&file_mtime);
}

void ConvertSecondsToFileInfoTime(DATETIME* pDateTime, unsigned long ts)
{
    struct tm *timeptr;
    time_t t = ts;
...
#ifndef NO_UTIME
	//istat.st_mtime
    time_stamp = (no_time && !list) ? 0 : ConvertFileInfoTimeToLong(&istat.st_mtime);
#endif

    /* Generate output file name. For -r and (-t or -l), skip files
     * without a valid gzip suffix (check done in make_ofname).
     */
    if (to_stdout && !list && !ztest) {
		strcpy(ofname, "stdout");
...
//			fprintf(stderr, "%s: %s compressed to %s\n", progname, ifname, ofname);
    ...
    fclose(ifd);
    if (!to_stdout && !fclose(ofd)) {	//expected UNIX ok=0 but fclose() ok=1
		write_error();
    }
    if (method == -1) {
...
{
	char flags[3];
    FILEINFO ostat; /* stat for ofname */
 ...
 */
local int do_stat(char *name, FILEINFO *sbuf)
//    char *name;
//    struct stat *sbuf;
{
    errno = 0;
    return fstat(name, sbuf);
}
...
/*
local char *set_suffix(char *name)
{
	int nlen, slen, i;
    nlen = strlen(name);
	// if there is a period separator then append "gz", else append ".gz"
	for (i=nlen; i>0; i--)
	{
		if (name[i] == '.')
		{
 ...
 * Return OK or ERROR.
 */
local int get_istat(char *iname, FILEINFO *sbuf)
//    char *iname;
//    struct stat *sbuf;
{
...
local int make_ofname(void)
{
    ...
    if (decompress)
	{
		if (suff == NULL)
		{
			...
			if (!recursive && (list || ztest)) return OK;
 ...
 */
local int get_method(FILEHANDLE in)
 ...
 */
local void do_list(FILEHANDLE ifd, int method)
...
//    int method;  /* compression method */
{
	...
	else if (method < 0)
	{
		...
		display_ratio(total_out-(total_in-header_bytes), total_out, 1);
    ...
    if (method == DEFLATED && !last_member) {
         ...
         */
        bytes_in = (long)lseek(ifd, (-8), SEEK_END);	// was...(off_t)(-8)
        if (bytes_in != -1L)
		{
            uch buf[8];
            bytes_in += 8L;
            if (fread((char*)buf, sizeof(buf), sizeof(buf), ifd) != sizeof(buf))
			{
                read_error();
            }
            crc = LG(buf);
    ...
    display_ratio(bytes_out-(bytes_in-header_bytes), bytes_out, 1);
    printf(" %s\n", ofname);
}

bool SameTime(DATETIME *pa, DATETIME *pb)
{
    return (pa->wYear == pb->wYear
    	&& pa->wMonth == pb->wMonth
    	&& pa->wDay == pb->wDay
    	&& pa->wHour == pb->wHour
    	&& pa->wMinute == pb->wMinute);
}

/* ========================================================================
 * Return true if the two stat structures correspond to the same file.
 */
local int same_file(FILEINFO *stat1, FILEINFO *stat2)
//    struct stat *stat1;
//    struct stat *stat2;
{
	...
	&& SameTime(&(stat1->st_atime), &(stat2->st_atime))
	&& SameTime(&(stat1->st_mtime), &(stat2->st_mtime))
	&& SameTime(&(stat1->st_ctime), &(stat2->st_ctime))
#endif
    ;
}
 ...
 */
local int name_too_long(char *name, FILEINFO *statb)
//    char *name;           /* file name to check */
//    struct stat *statb;   /* stat buf for this file name */
{
    int s = strlen(name);
    char c = name[s-1];
    FILEINFO tstat; /* stat for truncated name */
    int res;

    tstat = *statb;      /* Just in case OS does not fill all fields */
    name[s-1] = '\0';
    res = findfirst(name, &tstat) == 0 && same_file(statb, &tstat);
    name[s-1] = c;
...
local int check_ofname()
{
    ...
    /* don't Ask permission to overwrite the existing file */
...
*/
local void reset_times (char *name, FILEINFO *fileinfo)
//    char *name;
//    struct stat *statb;
{
//    struct utimbuf timep;
	
	setprop(name, fileinfo, SFS_SET_WRITETIME);
 ...
 */
local void copy_stat(FILEINFO *ifstat)
//    struct stat *ifstat;
{
#ifndef NO_UTIME
	time_t modificationTime = ConvertFileInfoTimeToLong(&(ifstat->st_mtime));
	
    if (decompress && (time_stamp != 0) && (modificationTime != time_stamp))
	{
		ConvertSecondsToFileInfoTime(&(ifstat->st_mtime), time_stamp);
		if (verbose > 1)
		{
...
//    int exitcode;
{
	...
	*/
    in_exit = 0;
    // don't do this in tasking environment...was...exit(exitcode);
}
...
// Shell commands for NXC
//    {"zip",        	"z",    2,      0,      "src_filename, dest_filename - GZip a file",            	com_gzip},
//    {"unzip",       "uz",   2,      0,      "src_filename, dest_filename - UnGZip a file",            	com_ugzip},
int com_gzip(struct command *com)
{
	char cShellMsg[128];
    if(com->nargs == 1)
    {
        sprintf(cShellMsg, "GZip for %s\r\n", com->args[0]);
        ShellPutString(com->stdout, cShellMsg);
        gzsingle(com->args[0], 0);
    }
    else if(com->nargs > 1)
    {
        sprintf(cShellMsg, "GZip for %s\r\n", com->args[0]);
        ShellPutString(com->stdout, cShellMsg);
        gzmain(com->nargs, com->args);
    }
    return ESUCCESS;
}

int com_ugzip(struct command *com)
{
	char cShellMsg[128];
        if(com->nargs == 1)
    {
        sprintf(cShellMsg, "unGZip for %s\r\n", com->args[0]);
        ShellPutString(com->stdout, cShellMsg);
        gzsingle(com->args[0], 1);
    }
    else if(com->nargs >= 1)
    {
        gzmain(com->nargs, com->args);
