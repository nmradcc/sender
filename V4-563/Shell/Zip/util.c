 ...
 */
int copy(FILEHANDLE in, FILEHANDLE out)
 ...
 * for error return.
 */
void write_buf(FILEHANDLE fd, voidp buf, unsigned cnt)
//    int       fd;
//    voidp     buf;
//    unsigned  cnt;
{
...
{
    fprintf(stderr, "\n%s: %s: %s\n", progname, ifname, m);
//tmp quikie attempt at fix...    abort_gzip();
}
 ...
 */
void display_ratio(long num, long den, int file)
//    long num;
//    long den;
//    FILE *file;
{
    ...
    fprintf(file, "%c%2ld.%1ld%%", cS, ratio / 10L, ratio % 10L);
}
...
void fprintf(int nDestCode, const char *format, ...)
{
	char temp[256];
