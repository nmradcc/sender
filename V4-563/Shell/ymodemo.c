              ...
              else
              {
                ...
                if (packets_received == 0)
                {
                  ...
                  if (packet_data[PACKET_HEADER] != 0)
                  {
                    ...
                    ret = f_open(&fp, (char*)file_name, FA_READ);
                	if(ret != FR_OK)
                	{
                		/* End session */
                		Send_Byte(CAN);
...
void Ymodem_PrepareIntialPacket(uint8_t *data, const uint8_t* fileName, uint32_t *length)
{
  ...
  sprintf((char*)file_ptr, "%lu", *length);
...
#define PACKET_TIMEOUT	NAK_TIMEOUT

uint8_t YmPort;


static char _getchar(uint32_t timeout)
{
	int8_t ch;
