...
//__ALIGN_BEGIN static UCHAR  shell_byte_pool_buffer[SHELL_APP_MEM_POOL_SIZE] __ALIGN_END;
//static TX_BYTE_POOL shell_app_byte_pool;

//__ALIGN_BEGIN static UCHAR  script_byte_pool_buffer[SCRIPT_APP_MEM_POOL_SIZE] __ALIGN_END;
//static TX_BYTE_POOL script_app_byte_pool;
...
int ShellMain(uint8_t bPort, char* buf)
{
	...
	else
	{
		if(RunScript(bPort, args[0], nargs, args) == 0)
		{
			if(strlen(buf) != 0)
			{
//k				res = OpenPacket(&fp, szPathVar, args[0]);
//k				if(res == FR_OK)
//k				{
//k					fclose(&fp);
