...
//k        else if(HAL_SD_ConfigWideBusOperation(&hsd_sdmmc[Instance], SDMMC_BUS_WIDE_4B) != HAL_OK)
//k        {
//k          ret = BSP_ERROR_PERIPH_FAILURE;
//k        }
//k        else
//k        {
//k          /* Switch to High Speed mode if the card support this mode */
//k          (void)HAL_SD_ConfigSpeedBusOperation(&hsd_sdmmc[Instance], SDMMC_SPEED_MODE_HIGH);
